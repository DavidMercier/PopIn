...
if gui.flag.flag_data == 0
    ...
    if gui.flag.flag_cleaned_data == 1
        ...
        for ii = 1:(length(gui.data_xls.sheets_xls)-1),
            gui.results(ii+1, 1).prob = (((gui.results(ii, 1).prob*(length(gui.data_xls.sheets_xls)+1))+1)/(length(gui.data_xls.sheets_xls)+1));
        end
            ...
            for ii_sheet = 1:1:length(gui.data_xls.sheets_xls)
                ...
                if gui.settings.set_popin == 1
                    if isempty(ind_popin_all)
                        ind_popin_all(1) = length(gui.data(ii_sheet).data_h_cleaned(:, 1));
                    end
                    ...
                    if length(ind_popin_all) == 1
                        ind_popin_all(2) = length(gui.data(ii_sheet).data_h_cleaned(:, 1));
                    end
                ...
                if gui.settings.value_crit_param == 1
                    gui.data(ii_sheet).sum_L = gui.data(ii_sheet).data_L_cleaned(ind_popin);
                    gui.data(ii_sheet).mean_sum_L = mean(gui.data(ii_sheet).sum_L);
                    
                elseif gui.settings.value_crit_param == 2
                    gui.data(ii_sheet).sum_h = gui.data(ii_sheet).data_h_cleaned(ind_popin);
                    gui.data(ii_sheet).mean_sum_h = mean(gui.data(ii_sheet).sum_h);
